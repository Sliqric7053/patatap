...
function init() {
  ...
  function onKeyDown(event) {
    if (keyData[event.key]) {
      //define a new point equals the size (h * w) of the current window
      var maxPoint = new Point(view.size.width, view.size.height);
      //get a random point
      var randomPoint = Point.random();
      //randomize point within the param of the maxPoint
      var point = maxPoint * randomPoint;
      var newCircle = new Path.Circle(point, 300);
      //assign color to the corresponding "key" that was pressed
      newCircle.fillColor = keyData[event.key].color;
      //play the corresponding "key" that was pressed
  ...
  function onFrame(event) {
    //for each frame, change the fill color of each circle slightly by adding 1 to its hue:
